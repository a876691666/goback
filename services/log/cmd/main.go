...
func main() {
	...
	// 注册路由
	opLogCtrl.RegisterRoutes(app, jwtMiddleware)
	loginLogCtrl.RegisterRoutes(app, jwtMiddleware)
	...
	// 服务地址
	addr := fmt.Sprintf("%s:%d", cfg.Server.HTTP.Host, 8085)

	// 注册服务
	svc := pkgRegistry.NewServiceBuilder("log-service", "v1.0.0").
		WithAddress(addr).
		WithBasePath("logs").
		Build()
