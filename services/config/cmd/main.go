...
func main() {
	...
	err := lifecycle.New(serviceName).
		...
		OnStart(func(s *lifecycle.Service) error {
			...
			sysConfigCtrl := &sysconfig.Controller{BaseController: baseCtrl}

			middlewares := map[string]fiber.Handler{
				"jwt": jwtMiddleware,
			}
			router.Register(app, middlewares, sysConfigCtrl)
			return nil
		}).
		OnReady(func(s *lifecycle.Service) error {
			logger.Info("系统参数配置服务就绪", zap.String("addr", addr))
			return nil
		}).
		OnStop(func(s *lifecycle.Service) error {
			logger.Info("系统参数配置服务正在清理资源...")
			return nil
		}).
